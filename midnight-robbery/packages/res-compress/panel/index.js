        ...
        plugin = new window.Vue({
            ...
            methods: {
                ...
                onBtnClickCompressAllMusic () {
                    console.log("Compress the entire project audio file");
                    this._compressMp3(this.mp3Array);
                },
                onBtnClickCompressAllImage () {
                    console.log("Compress the entire project image file");
                ...
                _compressImage (dataArr) {
                    (async () => {
                        for (let i = 0; i < dataArr.length; i++) {
                            ...
                            if (file) {
                                this._addLog(` Compression succeeded [${(i + 1)}/${dataArr.length}]: ${data.url}`);
                                // 导入到项目原位置
                                ...
                                this._addLog(`Image compression failed:${data.path}`)
                            }
                        }
                        this._addLog("Compressed!");
                    })();
                },
                    ...
                    (async () => {
                        ...
                        for (let i = 0; i < fileDataArray.length; i++) {
                            ...
                            if (!Fs.existsSync(voiceFile)) {
                                this._addLog("sound file does not exist: " + voiceFile);
                                return;
                            }
                            ...
                            if (Path.extname(voiceFile) === ".mp3") {
                                ...
                                await child_process.execPromise(cmd, null, (err) => {
                                    this._addLog("An error occurred: \n" + err);
                                });
                                // 临时文件重命名
                                let newNamePath = Path.join(tempMp3Dir, fileName + '.mp3');
                                Fs.renameSync(tempMp3Path, newNamePath);
                                this._addLog(`Compression succeeded [${(i + 1)}/${fileDataArray.length}] : ${voiceFileUrl} `);
                                ...
                                console.log("unsupported file type:" + voiceFile);
                            }
                        }
                        this._addLog("processed!");
                    })();
                },
